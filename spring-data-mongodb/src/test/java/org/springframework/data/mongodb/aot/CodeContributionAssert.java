...
public class CodeContributionAssert { // extends AbstractAssert<CodeContributionAssert, CodeContribution>
	...
	// assertThat(jdkProxiesFor(entryPoint).findFirst()).describedAs("Found jdk proxy matching %s though it should not be
	// present.", entryPoint).isNotPresent();
	// return this;
	// }
	//
	// public CodeContributionAssert doesNotContributeJdkProxy(Class<?>... proxyInterfaces) {
	//
	// assertThat(jdkProxiesFor(proxyInterfaces[0])).describedAs("Found jdk proxy matching %s though it should not be
	// present.", Arrays.asList(proxyInterfaces)).noneSatisfy(it -> {
	...
	//
	// assertThat(jdkProxiesFor(proxyInterfaces[0])).describedAs("Unable to find jdk proxy matching %s",
	// Arrays.asList(proxyInterfaces)).anySatisfy(it -> {
	// new JdkProxyAssert(it).matches(proxyInterfaces);
	// });
	//
	// return this;
	...
	//
	// assertThat(classProxiesFor(proxyInterfaces[0])).describedAs("Unable to find jdk proxy matching %s",
	// Arrays.asList(proxyInterfaces)).anySatisfy(it -> {
	...
	// }
	//
	// public void contributesClassProxy(Class<Address> addressClass, Class<LazyLoadingProxy> lazyLoadingProxyClass) {
	//
	// }
}
